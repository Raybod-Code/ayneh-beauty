...
import { Mail, Lock, Loader2, CheckCircle, RotateCcw, ArrowLeft, Eye, EyeOff } from "lucide-react";
...
function AynehShader() {
  ...
  useEffect(() => {
    ...
    const frag = `precision mediump float;uniform float u_time;uniform vec2 u_res;varying vec2 v_uv;
      float hash(vec2 p){return fract(sin(dot(p,vec2(127.1,311.7)))*43758.5453);}
      float noise(vec2 p){vec2 i=floor(p),f=fract(p);f=f*f*(3.0-2.0*f);return mix(mix(hash(i),hash(i+vec2(1,0)),f.x),mix(hash(i+vec2(0,1)),hash(i+vec2(1,1)),f.x),f.y);}
      float fbm(vec2 p){float v=0.0,a=0.5;mat2 r=mat2(0.8,0.6,-0.6,0.8);for(int i=0;i<6;i++){v+=a*noise(p);p=r*p*2.1;a*=0.48;}return v;}
      void main(){float t=u_time*0.07;vec2 uv=v_uv;
        ...
        col*=clamp(vig*1.15,0.0,1.0)*0.82;gl_FragColor=vec4(col,1.0);}`;
...
function FieldInput({
  value, onChange, type = "text", placeholder, icon, showToggle
}: {
  ...
  return (
    <div style={{
      ...
      <span style={{ color: focused ? "rgba(198,168,124,0.8)" : "rgba(255,255,255,0.2)", flexShrink:0, transition:"color 0.3s", display:"flex" }}>
        {icon}
      </span>
      <input
        type={showToggle ? (show ? "text" : "password") : type}
        dir="ltr" placeholder={placeholder}
        ...
        style={{
          ...
          fontFamily: type === "password" ? "monospace" : "'SF Mono','Fira Code','Courier New',monospace",
          color:"rgba(255,255,255,0.9)", letterSpacing:"0.04em", caretColor:"#C6A87C",
        }}
      />
      {showToggle && (
        <button type="button" onClick={() => setShow(s => !s)}
          style={{ background:"none", border:"none", cursor:"pointer", color:"rgba(255,255,255,0.2)", display:"flex", padding:0, flexShrink:0 }}>
...
function FormPanel({
  step, email, setEmail, loading, error, onSubmitMagic, onSubmitPassword, onReset
}: {
  step: "email" | "sent";
  email: string; setEmail: (v: string) => void;
  loading: boolean; error: string;
  onSubmitMagic: (e: React.FormEvent) => void;
  onSubmitPassword: (e: React.FormEvent, password: string) => void;
  onReset: () => void;
}) {
  const [mode, setMode] = useState<"magic" | "password">("magic");
  const [password, setPassword] = useState("");
  const valid = email.includes("@") && email.includes(".");
  const validPw = valid && password.length >= 4;
    ...
    <AnimatePresence mode="wait">
      {step === "email" ? (
        <motion.div key="email-step"
          ...
          <motion.span initial={{ opacity:0, scale:0.88 }} animate={{ opacity:1, scale:1 }}
            transition={{ delay:0.06 }} style={{
              display:"inline-flex", alignItems:"center", gap:8, fontSize:9,
          ...
          <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:0.14 }}
            ...
            {(["magic", "password"] as const).map(m => (
              <button key={m} onClick={() => setMode(m)} style={{
                ...
                fontSize:11, letterSpacing:"0.1em", cursor:"pointer", transition:"all 0.25s",
                borderColor: mode === m ? "rgba(198,168,124,0.5)" : "rgba(255,255,255,0.07)",
                background: mode === m ? "rgba(198,168,124,0.08)" : "rgba(255,255,255,0.02)",
                color: mode === m ? "#C6A87C" : "rgba(255,255,255,0.25)",
              }}>
                {m === "magic" ? "✉ لینک ایمیل" : "🔑 رمز عبور"}
              </button>
            ))}
          </motion.div>
          ...
          <motion.div initial={{ opacity:0, y:14 }} animate={{ opacity:1, y:0 }}
            transition={{ delay:0.18 }} style={{
              ...
              borderRadius:20, padding:26, backdropFilter:"blur(32px)", WebkitBackdropFilter:"blur(32px)",
            }}>

            <AnimatePresence mode="wait">
              {mode === "magic" ? (
                <motion.form key="magic" onSubmit={onSubmitMagic}
                  ...
                  <div>
                    ...
                    <FieldInput value={email} onChange={setEmail} type="email" placeholder="your@email.com" icon={<Mail size={14} />} />
                  </div>
                  <AnimatePresence>
                    {error && (
                      <motion.p initial={{ opacity:0, height:0 }} animate={{ opacity:1, height:"auto" }} exit={{ opacity:0, height:0 }}
                        style={{ fontSize:12, color:"rgba(248,113,113,0.75)", textAlign:"center" }}>{error}</motion.p>
                    )}
                  </AnimatePresence>
                  <motion.button type="submit" disabled={loading || !valid} whileTap={valid ? { scale:0.97 } : {}}
                    style={{
                      padding:"14px 20px", borderRadius:13, border:"none", fontWeight:600, fontSize:14,
                      letterSpacing:"0.05em", cursor:valid ? "pointer" : "not-allowed",
                      background: valid ? "linear-gradient(135deg,#C6A87C 0%,#E2C99A 50%,#B8955E 100%)" : "rgba(255,255,255,0.05)",
                    ...
                    {loading ? <Loader2 size={16} className="animate-spin" /> : <>ارسال لینک ورود →</>}
                  </motion.button>
                </motion.form>
              ) : (
                <motion.form key="password" onSubmit={e => onSubmitPassword(e, password)}
                  initial={{ opacity:0, x:10 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-10 }}
                  transition={{ duration:0.2 }}
                  style={{ display:"flex", flexDirection:"column", gap:14 }}>
                  <div>
                  ...
                  <div>
                    <label style={{ display:"block", fontSize:9, letterSpacing:"0.32em", textTransform:"uppercase", color:"rgba(255,255,255,0.16)", marginBottom:10 }}>رمز عبور</label>
                    <FieldInput value={password} onChange={setPassword} type="password" placeholder="••••••••" icon={<Lock size={14} />} showToggle />
                  </div>
                  <AnimatePresence>
                    {error && (
                      <motion.p initial={{ opacity:0, height:0 }} animate={{ opacity:1, height:"auto" }} exit={{ opacity:0, height:0 }}
                  ...
                  <motion.button type="submit" disabled={loading || !validPw} whileTap={validPw ? { scale:0.97 } : {}}
                    style={{
                      padding:"14px 20px", borderRadius:13, border:"none", fontWeight:600, fontSize:14,
                      letterSpacing:"0.05em", cursor:validPw ? "pointer" : "not-allowed",
                      background: validPw ? "linear-gradient(135deg,#C6A87C 0%,#E2C99A 50%,#B8955E 100%)" : "rgba(255,255,255,0.05)",
                      color: validPw ? "#0a0806" : "rgba(255,255,255,0.16)",
                      transition:"all 0.35s cubic-bezier(0.33,1,0.68,1)",
                      boxShadow: validPw ? "0 8px 28px rgba(198,168,124,0.28)" : "none",
                      display:"flex", alignItems:"center", justifyContent:"center", gap:8,
                    }}>
                    {loading ? <Loader2 size={16} className="animate-spin" /> : <>ورود به پنل →</>}
                  </motion.button>
                </motion.form>
              )}
            </AnimatePresence>
        ...
        <motion.div key="sent-step"
          ...
          style={{
            background:"rgba(255,255,255,0.025)", border:"1px solid rgba(255,255,255,0.07)",
            borderRadius:20, padding:"44px 28px", backdropFilter:"blur(32px)", WebkitBackdropFilter:"blur(32px)",
            textAlign:"center",
          }}>
...
export default function LoginPage() {
  ...
  const handleMagic = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError("مشکلی پیش اومد یا rate limit خوردی. از رمز عبور استفاده کن.");
    } finally {
      setLoading(false);
    }
  };

  const handlePassword = async (e: React.FormEvent, password: string) => {
    e.preventDefault();
    ...
    try {
      ...
      window.location.href = (role === "owner" || role === "superadmin") ? "/admin" : "/dashboard";
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : "";
      setError(msg.includes("Invalid") ? "ایمیل یا رمز اشتباهه." : "مشکلی پیش اومد.");
    } finally {
      setLoading(false);
    }
  };
